...
export default function LinkedInStudioPage() {
  ...
  const [repoSort, setRepoSort] = useState<'launches' | 'trending' | 'newest'>('launches');
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-bold text-slate-900 flex items-center gap-2 flex-wrap">
                    ...
                    {repoSort === 'launches'
                      ? 'Live Launch Radar: Freshly Dropped AI Agent Repos'
                    ...
                    <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[10px] font-bold border border-blue-200">
                      {repoSort === 'launches' ? (
                        <>
                          ...
                          <span className="text-emerald-700 font-extrabold">LIVE LAUNCH RADAR ({filteredRepos.length})</span>
                        </>
                      ) : (
                        <>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {repoSort === 'launches'
                    ? 'Our live radar catches brand-new open-source AI agent repositories as soon as they drop on GitHub. Spot new agent tools early and share fresh launch breakdowns on LinkedIn.'
              ...
              <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
                ...
                <div className="inline-flex p-1 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                  <button
                    onClick={() => {
                      setRepoSort('launches');
                      fetchRepos(repoSearchQuery, 'launches', activeRepoCategory);
                    }}
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${repoSort === 'launches' ? 'bg-white' : 'bg-emerald-500'} animate-pulse`}></span>
                ...
                return (
                  ...
                  >
                    <div className="space-y-2.5">
                      {/* Top Row: Language & Star Count + Fresh Launch Badge */}
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-slate-100 text-blue-700 truncate max-w-[130px] flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {repo.isFreshLaunch && (
                            <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-300 flex items-center gap-1 shadow-xs">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping"></span>
                              {repo.launchAgeText || 'Fresh Drop'}
                            </span>
                          )}
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                      ...
                      >
                        {isSelected ? (
                          ...
                          <>
                            <Zap className="w-3 h-3 text-emerald-200" /> Announce Launch
                          </>
                        ) : (
                          <>
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-blue-800 flex items-center gap-1.5">
                      {selectedRepo.isFreshLaunch ? (
                        <>
                          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></span>
                          <span className="text-emerald-700 font-extrabold">🚨 Fresh Launch Detected:</span>
                        </>
                      ) : (
                    ...
                    <p className="text-[11px] text-slate-500 truncate flex items-center gap-2">
                      {selectedRepo.launchAgeText && (
                        <span className="font-bold text-emerald-700">{selectedRepo.launchAgeText}</span>
                      )}
                      <span>•</span>
                  ...
                  >
                    {isGenerating ? (
                      <>
                        ...
                        Analyzing Launch & Crafting Post...
                      </>
                    ) : selectedRepo.isFreshLaunch ? (
                      <>
                        <Zap className="w-4 h-4 text-amber-300" />
                        Announce New Launch on LinkedIn
                      </>
                    ) : (
                      <>
